...
<body>
	<div class="bagcontent">
			<div class="kontent">
					<p class="pp"> Judul Manga </p>
					<hr>
	<?php
	$nomor = 0;
	$sql = "select * from tb_manga order by judul asc";
	$compair = mysqli_query($koneksi,$sql);
	while ($ulangi=mysqli_fetch_array($compair)){
		$nomor = $nomor+1;
		$id_mangas = $ulangi['id_manga'];
		$nama_manga = $ulangi['judul'];
	$sql2 = "select * from tb_manga where id_manga='$id_mangas'";
	$tampilkan = mysqli_query($koneksi,$sql2);
	$total = mysqli_num_rows($tampilkan);

	?> 
	<p><?php echo $nomor; ?>.
	<td><a href="info_manga.php?id_manga=<?php echo $id_mangas; ?>"><?php echo $nama_manga; ?></a></td>
	<?php } ?>
</div>
</div>
</body>